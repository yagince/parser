 ...
 */
public class Q5ParserTest extends ParserTestRoot<Expression> {
    ...
    /**
     * Test method for {@link jp.undo.vtable.parser.prep.Q5Parser#parse()}.
     * @throws Exception 任意の例外が発生した場合
     */
    @Test
     ...
     */
    @Test
    public void testParse_Plus() throws Exception {
        verify(
            test("単項プラステスト", "+ 1"),
            new Plus(new Value("1")));
    }
     ...
     */
    @Test
    public void testParse_Parenthesized() throws Exception {
        verify(
            test("括弧付き式テスト", "(1)"),
            new Parenthesized(new Value("1")));
    }
     ...
     */
    @Test
    public void testParse_AddSubAdd() throws Exception {
        verify(
            test("足して引いて足す", "1 + 2 - 3 + 4"),
            new Add(
                new Subtract(
                    new Add(new Value("1"), new Value("2")),
                    new Value("3")),
     ...
     */
    @Test
    public void testParse_MulDivMul() throws Exception {
        verify(
            test("掛けて割って掛ける", "1 * 2 / 3 * 4"),
            new Multiply(
                new Divide(
                    new Multiply(new Value("1"), new Value("2")),
                    new Value("3")),
     ...
     */
    @Test
    public void testParse_AddMulSub() throws Exception {
        verify(
            test("足して掛けて引く", "1 + 2 * 3 - 4"),
            new Subtract(
                new Add(
                    new Value("1"),
                    new Multiply(new Value("2"), new Value("3"))),
                new Value("4")));
    }
     ...
     */
    @Test
    public void testParse_AddParenSub() throws Exception {
        verify(
            test("足して括弧付きの引く", "1 + (2 - 3)"),
            new Add(
                new Value("1"),
                new Parenthesized(
                    new Subtract(
     ...
     */
    @Test
    public void testParse_DeepUnary() throws Exception {
        verify(
            test("単項演算子たくさん", "+ - + (1)"),
            new Plus(new Minus(new Plus(new Parenthesized(new Value("1"))))));
    }
     ...
     */
    @Test
    public void testParse_ExtraOpen() throws Exception {
        error("括弧開きすぎ", "(((1))");
    }
     ...
     */
    @Test
    public void testParse_ExtraClose() throws Exception {
        error("括弧閉じすぎ", "((1)))");
    }
    ...
    protected Expression parse(Reader input) throws IOException {
        Q5Parser parser = new Q5Parser(input);
        return parser.parse();
    }
}
